...
export default function UploadPanel({ tablas, token }) {
  ...
  const handleUpload = async () => {
    ...
    try {
      const res = await fetch(`${API}/upload/${tabla}`, {
        method: 'POST',
        body: form,
        headers: { Authorization: `Bearer ${token}` },
      })
      const data = await res.json()
      if (!res.ok) {
        setJob({ status: 'error', message: data.detail || `Error HTTP ${res.status}` })
        setLoading(false)
        return
      }
      if (data.job_id) {
        ...
        setJob({ status: 'error', message: data.detail || 'Error desconocido' })
        setLoading(false)
      }
    } catch (e) {
  ...
  return (
    <div className="card">
      ...
      <div className="filters" style={{ marginBottom: '1rem' }}>
        <label>
          Tipo documental
          <select value={tabla} onChange={e => { setTabla(e.target.value); setJob(null); setJobId(null); clearSelectedFile() }}>
      ...
      {job && (
        <div style={{ marginTop: '1rem' }}>
          ...
          <div className="progress-bar">
            <div style={{
              width: `${progress}%`,
              background: job.status === 'error' ? '#e53e3e' : job.status === 'done' ? '#38a169' : '#1a3a5c',
      ...
      <div style={{ marginTop: '1rem' }}>
        ...
        {job?.status === 'done' && (
          <button className="btn secondary" style={{ marginLeft: '0.5rem' }}
            onClick={() => { clearSelectedFile(); setJob(null); setJobId(null) }}>
            Nueva carga
          </button>
        )}
      </div>
      ...
      {job?.status === 'done' && job.result && (
        <div className="result-box" style={{ marginTop: '1rem' }}>
          ...
          {/* Descargar reportes de duplicados en BD */}
          {job.result.archivos_reporte?.duplicados_bd && (
            <details style={{ marginTop: '0.5rem' }}>
              <summary style={{ cursor: 'pointer', fontSize: '0.85rem', color: '#1a5490', fontWeight: 'bold' }}>
                📥 Duplicados encontrados en BD ({job.result.duplicados_bd})
              </summary>
              <div style={{ marginTop: '0.5rem', padding: '0.5rem', background: '#e3f2fd', borderRadius: 4 }}>
                <p style={{ fontSize: '0.8rem', margin: '0 0 0.5rem 0' }}>
                  Estos registros ya existían en la base de datos y fueron omitidos:
                </p>
                <button 
                ...
                >
                  Descargar duplicados_bd.xlsx
                </button>
              </div>
            </details>
          )}

          {/* Descargar reportes de duplicados en archivo */}
          {job.result.archivos_reporte?.duplicados_archivo && (
            <details style={{ marginTop: '0.5rem' }}>
              <summary style={{ cursor: 'pointer', fontSize: '0.85rem', color: '#ff8a00', fontWeight: 'bold' }}>
                📥 Registros repetidos en el archivo ({job.result.duplicados_archivo})
              </summary>
              <div style={{ marginTop: '0.5rem', padding: '0.5rem', background: '#fff8f0', borderRadius: 4 }}>
                <p style={{ fontSize: '0.8rem', margin: '0 0 0.5rem 0' }}>
                  Estas filas estaban repetidas dentro del archivo Excel cargado:
                </p>
                <button 
                  className="btn secondary"
                  style={{ fontSize: '0.8rem', padding: '0.4rem 0.8rem' }}
                ...
                >
                  Descargar duplicados_archivo.xlsx
                </button>
              </div>
            </details>
          )}
          ...
          {job.result.registros_duplicados?.length > 0 && (
            <details style={{ marginTop: '0.5rem' }}>
              <summary style={{ cursor: 'pointer', fontSize: '0.85rem', color: '#c05621', fontWeight: 'bold' }}>
                ⚠️ Ver detalles de registros duplicados ({job.result.registros_duplicados.length})
              </summary>
                  ...
                  <div key={idx} style={{ marginBottom: '0.3rem', paddingBottom: '0.3rem', borderBottom: '1px solid #ffe0cc' }}>
                    ...
                    <ul style={{ margin: '0.2rem 0', paddingLeft: '1rem' }}>
                      {Object.entries(reg).map(([k, v]) => (
                        <li key={k}><span style={{ color: '#666' }}>{k}:</span> <strong>{v ?? 'null'}</strong></li>
                      ))}
                    </ul>
                  </div>
                ))}
